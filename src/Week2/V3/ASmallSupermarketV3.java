 ...
 */
public class ASmallSupermarketV3 {
    public static void main(String[] args) {
        //创建超市实例并初始化
        SupermarketTemV3 supermarket = new SupermarketTemV3("黑店", "纽约市人民大道886号", 100, 100);
        //开始营业
        ...
        while (open) {
            ...
            //开始购物
            while (true) {
                System.out.println("请输入商品编号（从0-" + (supermarket.merchandiseV3.length - 1) + "）:");
                int index = scanner.nextInt();
                if (index == -1) {
                    System.out.println("再见，有请下一位顾客。");
                } else if (index < -1 || index > supermarket.merchandiseV3.length - 1) {
                    System.out.println("请输入正确范围内的商品编号：");
                    continue;
                } else {
                    System.out.println("请输入你要购买的数量：");
                    int num2buy = scanner.nextInt();
                    customer.CustomerBuy(num2buy, customer, supermarket.merchandiseV3[index]);
                }
            }
        }
    }
